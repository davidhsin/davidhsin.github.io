...
### 問題
根據題目給的提示，第1點和第2點其實都不是大問題，我認為比較難的點是 load balancing，應該怎麼合理的分配工作給不同的 Processes ？

起初設定了每個 process 都分配到一樣的 rows 數，但其實每個 row (或者說 pixel)的工作量差異甚大，這是導致 TLE 的主因。
![Screenshot 2024-08-14 at 11.43.58](https://hackmd.io/_uploads/HJcW_iY90.png)

工作分配不均的結果，就算AC也僅是壓線過：
![image](https://hackmd.io/_uploads/HkkProF50.png)

稍微觀察最內層迴圈可以發現，若要在圖片上塗上黑色 pixel，那麼必須跑完10001次迴圈，其餘顏色則不用跑那麼多。也就是說繁重的工作都在黑色部分。
...
這是第一筆測資（HEIGHT: 600 WIDTH: 800）共600 rows，可以發現愈靠近中間計算量愈大，甚至超過了三百萬。

![Screenshot 2024-08-14 at 17.20.29](https://hackmd.io/_uploads/BJlGePg5qR.png)
在row 301 處迴圈甚至要跑450萬次去處理僅800個 pixels。
...
#### 2. 憑感覺切 :)
想法非常的無腦，將總 rows 數以測資規定的 process 數平分給每個 process ，再將中間部分的工作量除以4，扣除的工作分給頭尾兩個負擔較輕的 process 。
...
一定有更完美的切法，但我不會🤷‍♂️。btw, Mandelbrot Set 裡面暗藏著 fibonacci sequence ，或許照著1 1 2 3 5 8的比例去切會有更好的效能？
...
#pragma omp parallel for collapse(2) schedule(dynamic)
    for (int y = start_row; y < end_row; y++) {
        for (int x = 0; x < WIDTH; x++) { 
            ...
            int repeats = 0;
            for (repeats = 0; repeats <= MaxRepeats; repeats++) { // 最多跑10001次
                z = z * z + c;
    ...
    // 非rank 0 process 將自己部分的計算結果傳給Main Process，
    if (rank != 0) {
        MPI_Send(
        ...
        // Main Process 將多個 process 的結果集合起來
...
[第10講 Embarrassingly Computations & Load Balancing](https://reurl.cc/vv0bgk)
